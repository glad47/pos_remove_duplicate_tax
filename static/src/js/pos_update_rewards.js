odoo.define('pos_loyalty_updating_reward.pos_polling', function (require) {
 ...
 Order.include({
            ...
            export_as_JSON() {
                    const json = Order.export_as_JSON(...arguments);
                ...
                init_from_JSON(json) {
                    ...
                    Order.init_from_JSON(...arguments);
                ...
                wait_for_push_order() {
                    return (!_.isEmpty(this.couponPointChanges) || this._get_reward_lines().length || Order.wait_for_push_order(...arguments));
                },
                /**
                 ...
                 */
                export_for_printing() {
                    const result = Order.export_for_printing(...arguments);
                    if (this.get_partner()) {
                ...
                _get_ignored_product_ids_total_discount() {
                    const productIds = Order._get_ignored_product_ids_total_discount(...arguments);
                ...
                get_orderlines() {
                    const orderlines = Order.get_orderlines(this, arguments);
                    const rewardLines = [];
                ...
                _get_reward_lines() {
                    const orderLines = Order.get_orderlines(...arguments);
                    // setInterval(() => {
                    //     console.log("xcvxcvxcvxcvxcvxcvxcvxcvxcvxcvxcvxcvxcvxcvxcv")
                    //     console.log(super.get_orderlines(...arguments))
                    // }, 3000)
                    if (orderLines) {
                ...
                get_last_orderline() {
                    const orderLines = Order.get_orderlines(...arguments).filter((line) => !line.is_reward_line);
                    return orderLines[orderLines.length - 1];
                },
                set_pricelist(pricelist) {
                    Order.set_pricelist(...arguments);
                    this._updateRewards();
                },
                set_orderline_options(line, options) {
                    Order.set_orderline_options(...arguments);
                ...
                _updateRewardLines() {
                    ...
                    for (const line of rewardLines) {
                        if(this.pos.reward_by_id[line.reward_id]){
                            const claimedReward = {
                ...
                _getGlobalDiscountLines() {
                    return this.get_orderlines().filter((line) => {
                        const reward = this.pos.reward_by_id[line.reward_id];
                        if (reward && line.reward_id) {
                           return reward && reward.is_global_discount;
                        }
                        return false
                    });
                },
